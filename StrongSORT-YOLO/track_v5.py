                    ...
                    for j, (output, conf) in enumerate(zip(outputs[i], confs)):
                        ...
                        if names[c] in filter:
                            ...
                            for i1 in range(1, len(trajectory[id])):
                                ...
                                thickness = 2
                                try:
                                    cv2.line(im0, int(trajectory[id][i1 - 1][0]) + int(trajectory[id][i1 - 1][2]) // 2,
                                             int(trajectory[id][i1][0]) + int(trajectory[id][i1][2]) // 2, (0, 0, 255),
                                             thickness)
                                except:
                                    pass
                                mx = max(trajectory[id][i1 - 1][0] - trajectory[id][i1][0],
                                         trajectory[id][i1 - 1][1] - trajectory[id][i1][1],
                                         trajectory[id][i1][0] - trajectory[id][i1 - 1][0],
                                         trajectory[id][i1][1] - trajectory[id][i1 - 1][1])
                                print(mx)
                                if mx<2:
                                    continue
                                if mx == trajectory[id][i1 - 1][0] - trajectory[id][i1][0]:
                                    move_type = "left"
                                elif mx == trajectory[id][i1][0] - trajectory[id][i1 - 1][0]:
                                    move_type = "right"
                                elif mx == trajectory[id][i1 - 1][1] - trajectory[id][i1][1]:
                                    move_type = "down"
                                elif mx == trajectory[id][i1 - 1][0] - trajectory[id][i1][0]:
                                    move_type = "up"

                                if move_type == plus_res and was[id]==minus_res:
                                        res += 1
                                elif move_type == minus_res and was[id]==plus_res:
                                        res -= 1
                                print(move_type, was[id], res)
                                was[id] = move_type
                            f = open("save.txt", "w")
...
def parse_opt():
    ...
    parser.add_argument('--yolo-weights', nargs='+', type=str, default=WEIGHTS / 'yolov5l.pt', help='model.pt path(s)')
    parser.add_argument('--plus-res', type=str, default='right', help='')
    parser.add_argument('--minus-res', type=str, default='left', help='')
